...
/**
 * ChromeNativeAdapter — connects AutoPriseAi to the user's already-running Chrome session.
 *
...
export class ChromeNativeAdapter {
  ...
  private async _tryChromeMCP(port: number): Promise<boolean> {
    try {
      ...
      const client = new Client({ name: 'AutoPriseAi', version: '10.0.0' }, { capabilities: {} });
