...
// ─── Core Initialization ────────────────────────────────────────────
console.log('[Server] Initializing AutoPriseAi v12.9.0...');
...
io.on('connection', (socket) => {
  ...
  socket.on('message', async (payload: { text: string; conversationId: string; image?: string }) => {
    ...
    try {
      ...
      try {
        await sendPushNotification('AutoPriseAi', preview, { type: 'chat_response', conversationId });
