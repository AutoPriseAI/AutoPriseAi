...
REQUIRES: AutoPriseAi Chrome extension relay connected.
...
Use dry_run: true to validate setup without posting.`,
  ...
  run: async (args: any, _meta: SkillMeta) => {
    ...
    if (!extensionRelay.connected) {
      const msg = 'Chrome extension relay not connected. Install and enable the AutoPriseAi Relay extension.';
      log('ERROR', msg);
      return { success: false, error: msg };
    }
