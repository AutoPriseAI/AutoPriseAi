...
/**
 * Unified browser skill — the ONLY browser tool for AutoPriseAi.
 *
 * THREE MODES:
 ...
 *   • extension-relay — connected via the AutoPriseAi Chrome extension for real DOM access.
 *
...
EXTENSION RELAY (AutoPriseAi Chrome Extension):
  ...
  run: async ({ action, url, target, text, code, port, args, tab_id }: {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'relay_tabs': {
          if (!extensionRelay.connected) return { success: false, error: 'Extension not connected. Install the AutoPriseAi Relay extension in Chrome.' };
