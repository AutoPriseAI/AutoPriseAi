# AutoPriseAi v12 — Final Implementation Plan
...
| Decision | Answer |
|---|---|
| Org creation | Both — Dashboard UI form + AutoPriseAi chat commands |
...
src/core/
  ...
  org-management-skill.ts   ← Skill for AutoPriseAi chat-based org management
...
```json
{
  ...
  "name": "AutoPriseAi Dev Team",
  "mission": "Build and maintain AutoPriseAi as the world's best local-first AI automation platform.",
  "rootDir": "C:/Projects/AutoPriseAi",
...
| FIX-F | 🟡 | AutoPriseAi chat-based org creation — Brain has no direct access to OrgManager | `manage_org` skill registered globally — Brain calls it as a tool. Also add slash command handlers in `index.ts` |
...
private initSession() {
  ...
  this.history = [
    ...
    { role: 'model', parts: [{ text: 'Online. AutoPriseAi v11 is ready. What do you need?' }] },
  ];
  this.turnCount = 0;
...
Registered globally — allows AutoPriseAi's regular Brain to manage orgs via chat.
...
export const orgManagementSkill: Skill = {
  name: 'manage_org',
  description: `Manage AutoPriseAi AI organisations and their agents. Actions:
      ...
      <div className="modal-panel modal-panel--wide" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. AutoPriseAi Dev Team" />
          </div>
          <div className="form-group">
...
#### New AutoPriseAi Skill (all chat panes)
...
79. **Org management via chat CRUD** — "list my orgs" → "pause the AutoPriseAi org" → "trigger Maya's heartbeat" → "add a new agent to org X" → all work via tool call
...
19. **`manage_org` skill registered in global skills index** — available to AutoPriseAi's regular Brain.
