...
| System | Before | After |
|---|---|---|
| Terminal output | Printed, lost on restart | Written to `logs/AutoPriseAi-{date}.log`, 7-day rolling |
...
│   ├── AutoPriseAi-2026-03-18.log     ← terminal output for that day
│   ├── AutoPriseAi-2026-03-19.log
...
```typescript
/**
 * AutoPriseAi Terminal Logger
 *
 * Tees all console output (log/warn/error) to a rolling daily log file.
 * Files live in logs/AutoPriseAi-{date}.log, auto-deleted after 7 days.
...
class TerminalLogger {
  ...
  start(): void {
    ...
    // Log startup marker
    const marker = `\n${'='.repeat(60)}\n  AutoPriseAi started at ${new Date().toISOString()}\n${'='.repeat(60)}\n`;
  ...
  private rotateIfNeeded(): void {
    ...
    const file = path.join(LOGS_DIR, `AutoPriseAi-${today}.log`);
  ...
  private cleanup(): void {
    try {
      ...
      for (const f of fs.readdirSync(LOGS_DIR)) {
        if (!f.startsWith('AutoPriseAi-') || !f.endsWith('.log')) continue;
...
### `logs/AutoPriseAi-2026-03-18.log`

```
============================================================
  AutoPriseAi started at 2026-03-18T09:00:00.123Z
============================================================
[2026-03-18T09:00:00.456Z] [INFO] [Server] Initializing AutoPriseAi v12...
...
[2026-03-18T09:00:05.678Z] [INFO] [OrgHeartbeat] ⏰ Heartbeat: Aria (CEO) in AutoPriseAi Enterprise
...
### Integration Tests
10. **Terminal log created** — start server → `logs/AutoPriseAi-{today}.log` exists
