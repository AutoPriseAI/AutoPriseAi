# AutoPriseAi Android App — Build Progress

**Last updated:** 2026-03-23 (session 2)
**App package:** `com.AutoPriseAi.app`
...
- Fixed package ID: `com.AutoPriseAi.AutoPriseAiApp` → `com.AutoPriseAi.app`
- Updated `android/app/build.gradle` namespace + applicationId
- Moved Kotlin files to `com/AutoPriseAi/app/` package
- Fixed all Expo SDK version mismatches to SDK 55
- Added `"main": "expo-router/entry"` to package.json
- Added scheme `"AutoPriseAi"` to app.json
...
**Error:** `Default FirebaseApp is not initialized in this process com.AutoPriseAi.app`
**Cause:** Expo push notifications on Android require Firebase Cloud Messaging (FCM). Need `google-services.json`.
**Fix:**
1. Go to [Firebase Console](https://console.firebase.google.com) → Create project
2. Add Android app with package `com.AutoPriseAi.app`
...
- Steps remaining:
  ...
  4. `cloudflared tunnel create AutoPriseAi`
  5. `cloudflared tunnel route dns AutoPriseAi api.utilization-tracker.online`
  6. `cloudflared tunnel run --url http://localhost:3000 AutoPriseAi`
...
2. Add Android app → package name: `com.AutoPriseAi.app`
3. Download `google-services.json` → place in `AutoPriseAiApp/android/app/`
4. Rebuild: `npx expo run:android`
