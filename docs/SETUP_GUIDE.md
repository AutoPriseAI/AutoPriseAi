# AutoPriseAi v12.8 — Complete Setup Guide

Everything you need to get AutoPriseAi running on your machine, including the optional Android app and remote access.
...
### Clone & Setup
```bash
git clone https://github.com/sagarkalra-tech/AutoPriseAi.git
cd AutoPriseAi
setup.bat
```
...
The extension lets AutoPriseAi interact with your real Chrome tabs (not just its own Playwright browser).
...
5. Type `/relay` in AutoPriseAi chat to verify

---

## 3. Telegram Bot (Optional)

Control AutoPriseAi from your phone via Telegram — useful for quick commands when you're away from your desk.
...
Full mobile app for controlling AutoPriseAi from your phone — chat, manage orgs, push notifications, voice input.
...
2. Add an **Android app** with package name `com.AutoPriseAi.app`
3. Download `google-services.json` → place it in `AutoPriseAiApp/android/app/`
...
4. **Create a tunnel**:
   ```bash
   cloudflared tunnel create AutoPriseAi
   ```
   Note the **Tunnel ID** from the output.

5. **Add a DNS record**:
   ```bash
   cloudflared tunnel route dns AutoPriseAi api.yourdomain.com
   ```
...
7. **Start the tunnel**:
   ```bash
   cloudflared tunnel run AutoPriseAi
   ```
...
### Verify
- Visit `https://api.yourdomain.com/status` in a browser — you should see the AutoPriseAi status JSON
...
| `SDK location not found` | Create `AutoPriseAiApp/android/local.properties` with `sdk.dir=C\:\\Users\\<you>\\AppData\\Local\\Android\\Sdk` |
